...
from transformers import (
    PartCLIPSegConfig,
    PartCLIPSegForImageSegmentation,
    PartCLIPSegProcessor,
    PartCLIPSegTextConfig,
    PartCLIPSegVisionConfig,
    CLIPTokenizer,
    ViTFeatureExtractor,
)


def get_partclipseg_config(model_name):
    text_config = PartCLIPSegConfig()
    vision_config = PartCLIPSegVisionConfig(patch_size=16)

    use_complex_transposed_convolution = True if "refined" in model_name else False
    reduce_dim = 16 if "rd16" in model_name else 64

    config = PartCLIPSegConfig.from_text_vision_configs(
        text_config,
        vision_config,
...
def convert_partclipseg_checkpoint(model_name, checkpoint_path, pytorch_dump_folder_path, push_to_hub):
    config = get_partclipseg_config(model_name)
    model = PartCLIPSegForImageSegmentation(config)
    model.eval()
    ...
    processor = PartCLIPSegProcessor(feature_extractor=feature_extractor, tokenizer=tokenizer)
...
if __name__ == "__main__":
    ...
    convert_partclipseg_checkpoint(args.model_name, args.checkpoint_path, args.pytorch_dump_folder_path, args.push_to_hub)
