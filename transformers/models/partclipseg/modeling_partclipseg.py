...
from .configuration_partclipseg import PartCLIPSegConfig, PartCLIPSegTextConfig, PartCLIPSegVisionConfig
...
PARTCLIPSEG_PRETRAINED_MODEL_ARCHIVE_LIST = [
...
def partclipseg_loss(similarity: torch.Tensor) -> torch.Tensor:
...
class PartCLIPSegDecoderOutput(ModelOutput):
    ...
    part_obj_logits: torch.FloatTensor = None
...
class PartCLIPSegImageSegmentationOutput(ModelOutput):
    ...
    vision_model_output: BaseModelOutputWithPooling = None
    decoder_output: PartCLIPSegDecoderOutput = None
...
class PartCLIPSegVisionEmbeddings(nn.Module):
    ...
    def __init__(self, config: PartCLIPSegVisionConfig):
        super().__init__()
        self.config = config
...
# Copied from transformers.models.clip.modeling_clip.CLIPTextEmbeddings with CLIP -> CLIPSeg / PartCLIPSeg
class PartCLIPSegTextEmbeddings(nn.Module):
    def __init__(self, config: PartCLIPSegTextConfig):
        super().__init__()
...
class PartCLIPSegEncoderLayer(nn.Module):
    def __init__(self, config: PartCLIPSegConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.self_attn = PartCLIPSegAttention(config)
        self.layer_norm1 = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
        self.mlp = PartCLIPSegMLP(config)
...
class PartCLIPSegPreTrainedModel(PreTrainedModel):
    ...
    config_class = PartCLIPSegConfig
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, PartCLIPSegTextEmbeddings):
            module.token_embedding.weight.data.normal_(mean=0.0, std=factor * 0.02)
            module.position_embedding.weight.data.normal_(mean=0.0, std=factor * 0.02)
        elif isinstance(module, PartCLIPSegVisionEmbeddings):
            factor = self.config.initializer_factor
            nn.init.normal_(module.class_embedding, mean=0.0, std=module.embed_dim**-0.5 * factor)
            nn.init.normal_(module.patch_embedding.weight, std=module.config.initializer_range * factor)
            nn.init.normal_(module.position_embedding.weight, std=module.config.initializer_range * factor)
        elif isinstance(module, PartCLIPSegAttention):
            factor = self.config.initializer_factor
        ...
        elif isinstance(module, PartCLIPSegMLP):
            factor = self.config.initializer_factor
            in_proj_std = (
        ...
        elif isinstance(module, PartCLIPSegModel):
            nn.init.normal_(
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, PartCLIPSegEncoder):
            module.gradient_checkpointing = value


PARTCLIPSEG_START_DOCSTRING = r"""
...
PARTCLIPSEG_TEXT_INPUTS_DOCSTRING = r"""
    Args:
...
PARTCLIPSEG_VISION_INPUTS_DOCSTRING = r"""
    Args:
...
PARTCLIPSEG_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Padding will be ignored by default should you provide
            it.
...
class PartCLIPSegTextTransformer(nn.Module):
    ...
    def __init__(self, config: PartCLIPSegTextConfig):
        ...
        self.config = config
        embed_dim = config.hidden_size
        self.embeddings = PartCLIPSegTextEmbeddings(config)
        self.encoder = PartCLIPSegEncoder(config)
        self.final_layer_norm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)

    @add_start_docstrings_to_model_forward(PARTCLIPSEG_TEXT_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=PartCLIPSegTextConfig)
...
class PartCLIPSegTextModel(PartCLIPSegPreTrainedModel):
    config_class = PartCLIPSegTextConfig

    _no_split_modules = ["PartCLIPSegEncoderLayer"]

    def __init__(self, config: PartCLIPSegTextConfig):
        super().__init__(config)
        self.text_model = PartCLIPSegTextTransformer(config)
...
class PartCLIPSegVisionTransformer(nn.Module):
    ...
    def __init__(self, config: PartCLIPSegVisionConfig):
        ...
        self.embeddings = PartCLIPSegVisionEmbeddings(config)
        self.pre_layrnorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
        self.encoder = PartCLIPSegEncoder(config)
        self.post_layernorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)

    @add_start_docstrings_to_model_forward(PARTCLIPSEG_VISION_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=PartCLIPSegVisionConfig)
...
class PartCLIPSegVisionModel(PartCLIPSegPreTrainedModel):
    config_class = PartCLIPSegVisionConfig
    main_input_name = "pixel_values"

    def __init__(self, config: PartCLIPSegVisionConfig):
        super().__init__(config)
        self.vision_model = PartCLIPSegVisionTransformer(config)
        # Initialize weights and apply final processing
        self.post_init()
...
@add_start_docstrings(PARTCLIPSEG_START_DOCSTRING)
class PartCLIPSegModel(PartCLIPSegPreTrainedModel):
    config_class = PartCLIPSegConfig
    ...
    def __init__(self, config: PartCLIPSegConfig):
        ...
        if not isinstance(config.text_config, PartCLIPSegTextConfig):
            raise ValueError(
                "config.text_config is expected to be of type CLIPSegTextConfig but is of type"
                f" {type(config.text_config)}."
            )

        if not isinstance(config.vision_config, PartCLIPSegVisionConfig):
            raise ValueError(
        ...
        self.text_model = PartCLIPSegTextTransformer(text_config)
        self.vision_model = PartCLIPSegVisionTransformer(vision_config)
    ...
    @add_start_docstrings_to_model_forward(PARTCLIPSEG_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=PartCLIPSegOutput, config_class=PartCLIPSegConfig)
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        pixel_values: Optional[torch.FloatTensor] = None,
        ...
        loss = None
        if return_loss:
            loss = partclipseg_loss(logits_per_text)

        if not return_dict:
            output = (logits_per_image, logits_per_text, text_embeds, image_embeds, text_outputs, vision_outputs)
            return ((loss,) + output) if loss is not None else output

        return PartCLIPSegOutput(
            loss=loss,
...
class PartCLIPSegDecoderConcatSelfAttentionLayer(nn.Module):
    def __init__(self, input_dim, output_dim, n_heads=4):
        super(PartCLIPSegDecoderConcatSelfAttentionLayer, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.n_heads = n_heads
        self.query = nn.Linear(input_dim * 2, output_dim * n_heads)
        self.key = nn.Linear(input_dim * 2, output_dim * n_heads)
        self.value = nn.Linear(input_dim * 2, output_dim * n_heads)
        self.softmax = nn.Softmax(dim=-1)
        self.output_linear = nn.Linear(output_dim * n_heads, output_dim)

    def forward(self, combined_data):
        query = self.query(combined_data)
        key = self.key(combined_data)
        value = self.value(combined_data)

        batch_size = query.size(0)
        query = query.view(batch_size, -1, self.n_heads, self.output_dim)
        key = key.view(batch_size, -1, self.n_heads, self.output_dim)
        value = value.view(batch_size, -1, self.n_heads, self.output_dim)

        scores = torch.einsum('bnhq,bmhq->bhnm', query, key)
        weights = self.softmax(scores / (self.output_dim ** 0.5))

        weighted_values = torch.einsum('bhnm,bmhq->bnhq', weights, value)
        weighted_values = weighted_values.reshape(batch_size, -1, self.n_heads * self.output_dim)
...
class PartCLIPSegDecoder(PartCLIPSegPreTrainedModel):
    def __init__(self, config: PartCLIPSegConfig):
        ...
        self.cond_film_mul = nn.Linear(config.projection_dim, config.reduce_dim)
        self.cond_film_add = nn.Linear(config.projection_dim, config.reduce_dim)
        ...
        if config.use_complex_transposed_convolution:
            ...
            self.part_obj_with_cond_embed = nn.Sequential(
                nn.Conv2d(config.reduce_dim, config.reduce_dim, kernel_size=3, padding=1),
                nn.ReLU(),
                nn.ConvTranspose2d(
                    config.reduce_dim,
            ...
            self.part_obj_with_cond_embed = nn.ConvTranspose2d(
                config.reduce_dim, 1, config.vision_config.patch_size, stride=config.vision_config.patch_size
            )
        ...
        self.layers = nn.ModuleList([PartCLIPSegDecoderLayer(decoder_config) for _ in range(len(config.extract_layers))])
    ...
    def forward(
        ...
        num_text_classes: Optional[int] = None,
        num_part_classes: Optional[int] = None,
        part_index_map: Optional[torch.Tensor] = None,
        obj_index_map: Optional[torch.Tensor] = None,
    ):
        ...
        cond_emb_dim = conditional_embeddings.shape[1]
        size = int(math.sqrt(output.shape[2]))

        output_ = output.view(batch_size, -1, *output.shape[1:])
        conditional_embeddings_ = conditional_embeddings.view(batch_size, -1, cond_emb_dim)
        text_class_embeddings = torch.zeros((batch_size, num_text_classes, cond_emb_dim * 2), device=self.device)

        part_obj_output = torch.zeros((batch_size, num_text_classes, channel * 2, output.shape[2]), device=self.device)

        part_obj_output[..., :channel, :] += torch.gather(output_[:, :num_part_classes], 1, part_index_map.permute(0, 3, 1, 2).repeat(batch_size, 1, *output.shape[1:]))
        part_obj_output[..., channel:, :] += torch.gather(output_[:, num_part_classes:], 1, obj_index_map.permute(0, 3, 1 ,2).repeat(batch_size, 1, *output.shape[1:]))
        part_obj_output = torch.cat([part_obj_output, output_[:, -1].repeat(1, 2, 1).unsqueeze(1)], dim=1)
        part_obj_output = part_obj_output.view(batch_size * (num_text_classes + 1), -1, size, size)

        b = conditional_embeddings.shape[0]
        output = output.view(b, output.shape[1], size, size)
        ...
        #     part with object
        # ------------------------
        part_obj_ = self.part_obj_mlp(part_obj_output.permute(0, 2, 3, 1))

        text_class_embeddings[..., cond_emb_dim:] += torch.gather(conditional_embeddings_, 1, part_index_map.permute(0, 3, 1, 2).squeeze(-1).repeat(batch_size, 1, cond_emb_dim))
        text_class_embeddings[..., :cond_emb_dim] += torch.gather(conditional_embeddings_, 1, obj_index_map.permute(0, 3, 1, 2).squeeze(-1).repeat(batch_size, 1, cond_emb_dim))
        text_class_embeddings = torch.cat([text_class_embeddings, conditional_embeddings_[:, -1].repeat(1, 2).unsqueeze(1)], dim=1)

        reduced_cond_emb = self.reduce_cond_embed(text_class_embeddings)
        reduced_cond_emb = reduced_cond_emb.view(batch_size * (num_text_classes + 1), -1).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, size, size).permute(0, 2, 3, 1)

        part_obj_film = self.cond_film_mul(reduced_cond_emb) * part_obj_ + self.cond_film_add(reduced_cond_emb)
        part_obj_logits = self.part_obj_with_cond_embed(part_obj_film.permute(0, 3, 1, 2)).squeeze()

        if not return_dict:
            return tuple(v for v in [logits, part_obj_logits, all_hidden_states, all_attentions] if v is not None)

        return PartCLIPSegDecoderOutput(
            logits=logits,
            part_obj_logits=part_obj_logits,
...
class PartCLIPSegForImageSegmentation(PartCLIPSegPreTrainedModel):
    ...
    def __init__(self, config: PartCLIPSegConfig):
        ...
        self.clip = PartCLIPSegModel(config)
        self.extract_layers = config.extract_layers

        self.decoder = PartCLIPSegDecoder(config)
    ...
    @add_start_docstrings_to_model_forward(PARTCLIPSEG_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=PartCLIPSegImageSegmentationOutput, config_class=PartCLIPSegTextConfig)
    def forward(
        self,
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        num_text_classes: Optional[int] = None, # add num_text_classes
        num_part_classes: Optional[int] = None, # add num_part_classes
        part_index_map: Optional[torch.LongTensor] = None, # add part_index_map
        obj_index_map: Optional[torch.LongTensor] = None, # add obj_index_map
    ) -> Union[Tuple, PartCLIPSegOutput]:
        """
        ...
        batch_size, h, w = activations[0].shape[0], activations[0].shape[1], activations[0].shape[2]
        for i in range(len(activations)):
            activations[i] = activations[i].unsqueeze(1).repeat(1,len(conditional_embeddings),1,1).reshape(-1,h,w)
        conditional_embeddings = conditional_embeddings.unsqueeze(0).repeat(batch_size,1,1).reshape(-1, 512)
        ...
        part_obj_logits = decoder_outputs.part_obj_logits if return_dict else decoder_outputs[1]

        logits = logits.reshape(batch_size, -1, logits.shape[-2], logits.shape[-1])
        part_obj_logits = part_obj_logits.reshape(batch_size, -1, part_obj_logits.shape[-2], part_obj_logits.shape[-1])
        ...
        if not return_dict:
            output = (logits, part_obj_logits, conditional_embeddings, pooled_output, vision_outputs, decoder_outputs)
            return ((loss,) + output) if loss is not None else output

        return PartCLIPSegImageSegmentationOutput(
            loss=loss,
            logits=logits,
            part_obj_logits=part_obj_logits,
