...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        v = self.v(x[:, :, :self.dim]).reshape(
            B_, N, self.num_heads, -1).permute(0, 2, 1, 3)
        ...
        if mask is not None:
            ...
            attn = attn.view(B_ // num_win, num_win, self.num_heads,
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = WindowAttention(
            dim, appearance_guidance_dim=appearance_guidance_dim, num_heads=num_heads, head_dim=head_dim, window_size=to_2tuple(
                self.window_size),
    ...
    def forward(self, x, appearance_guidance):
        ...
        x_windows = x_windows.view(-1, self.window_size *
                                   self.window_size, x_windows.shape[-1])
...
class SwinTransformerBlockWrapper(nn.Module):
    def __init__(self, dim, appearance_guidance_dim, input_resolution, nheads=4, window_size=5):
        super().__init__()
        self.block_1 = SwinTransformerBlock(dim, appearance_guidance_dim, input_resolution,
                                            num_heads=nheads, head_dim=None, window_size=window_size, shift_size=0)
        self.block_2 = SwinTransformerBlock(dim, appearance_guidance_dim, input_resolution,
                                            num_heads=nheads, head_dim=None, window_size=window_size, shift_size=window_size // 2)
        self.guidance_norm = nn.LayerNorm(
            appearance_guidance_dim) if appearance_guidance_dim > 0 else None
    ...
    def forward(self, x, appearance_guidance):
        ...
        if appearance_guidance is not None:
            appearance_guidance = self.guidance_norm(
                repeat(appearance_guidance, 'B C H W -> (B T) (H W) C', T=T))
...
class LinearAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        queried_values = torch.einsum(
            "nlhd,nhdv,nlh->nlhv", Q, KV, Z) * v_length
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x, guidance):
        ...
        q = self.q(torch.cat([x, guidance], dim=-1)
                   ) if guidance is not None else self.q(x)
        k = self.k(torch.cat([x, guidance], dim=-1)
                   ) if guidance is not None else self.k(x)
        v = self.v(x)
...
class ClassTransformerLayer(nn.Module):
    def __init__(self, hidden_dim=64, guidance_dim=64, nheads=8, attention_type='linear', pooling_size=(4, 4)) -> None:
        ...
        self.attention = AttentionLayer(
            hidden_dim, guidance_dim, nheads=nheads, attention_type=attention_type)
    ...
    def forward(self, x, guidance):
        ...
        *_, H_pool, W_pool = x_pool.size()
        ...
        if guidance is not None:
            guidance = repeat(
                guidance, 'B T C -> (B H W) T C', H=H_pool, W=W_pool)

        x_pool = x_pool + \
            self.attention(self.norm1(x_pool), guidance)  # Attention
        x_pool = x_pool + self.MLP(self.norm2(x_pool))  # MLP

        x_pool = rearrange(
            x_pool, '(B H W) T C -> (B T) C H W', H=H_pool, W=W_pool)
        x_pool = F.interpolate(x_pool, size=(
...
class AggregatorLayer(nn.Module):
    def __init__(self, hidden_dim=64, text_guidance_dim=512, appearance_guidance=512, nheads=4, input_resolution=(20, 20), pooling_size=(5, 5), window_size=(10, 10), attention_type='linear') -> None:
        super().__init__()
        self.swin_block = SwinTransformerBlockWrapper(
            hidden_dim, appearance_guidance, input_resolution, nheads, window_size)
        self.attention = ClassTransformerLayer(
            hidden_dim, text_guidance_dim, nheads=nheads, attention_type=attention_type, pooling_size=pooling_size)
...
class AggregatorResNetLayer(nn.Module):
    def __init__(self, hidden_dim=64, appearance_guidance=512) -> None:
        ...
        self.conv_linear = nn.Conv2d(
            hidden_dim + appearance_guidance, hidden_dim, kernel_size=1, stride=1)
    ...
    def forward(self, x, appearance_guidance):
        ...
        x = rearrange(x, 'B C T H W -> (B T) C H W')
        appearance_guidance = repeat(
            appearance_guidance, 'B C H W -> (B T) C H W', T=T)
...
class Aggregator(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(prompt_channel, hidden_dim,
                               kernel_size=7, stride=1, padding=3)

        self.guidance_projection = nn.Sequential(
            nn.Conv2d(appearance_guidance_dim, appearance_guidance_proj_dim,
        ...
        self.decoder1 = Up(
            hidden_dim, decoder_dims[0], decoder_guidance_proj_dims[0])
        self.decoder2 = Up(
    ...
    def feature_map(self, img_feats, text_feats):
        ...
        img_feats = repeat(img_feats, "B C H W -> B C T H W",
        ...
        text_feats = repeat(text_feats, "B T C -> B C T H W",
                            H=img_feats.shape[-2], W=img_feats.shape[-1])
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        projected_guidance, projected_text_guidance, projected_decoder_guidance = None, None, [
            None, None]
        ...
        if self.decoder_guidance_projection is not None:
            projected_decoder_guidance = [proj(g) for proj, g in zip(
                self.decoder_guidance_projection, appearance_guidance[1:])]
