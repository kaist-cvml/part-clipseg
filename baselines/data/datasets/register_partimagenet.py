...
OBJ_BASE_CLASS_NAMES = [
    c for c in OBJ_CLASS_NAMES if c not in OBJ_NOVEL_CLASS_NAMES
]
...
BASE_CLASS_NAMES = [
    c for i, c in enumerate(CLASS_NAMES) if c.split('\'s', maxsplit=1)[0] not in OBJ_NOVEL_CLASS_NAMES
]
...
def load_json(_root, image_root, json_file, extra_annotation_keys=None, per_image=False, val_all=False, data_list=None):
    dataset_dicts = []
    obj_dataset_dicts = load_coco_json(os.path.join(_root, json_file), os.path.join(
        _root, image_root), extra_annotation_keys=extra_annotation_keys)
    ...
    for name, (image_root, json_file) in _PART_IN.items():
        ...
        if 'train' in name:
            DatasetCatalog.register(
                all_name, lambda: load_json(
                    root, "images/train", "train.json", per_image=True)
            )
            DatasetCatalog.register(
                name_obj_cond, lambda: load_json(
                    root, "images/train", "train.json")
            )
        else:
            DatasetCatalog.register(
                # name_obj_cond, lambda: load_train_val_json(root, "images/val", "val.json", per_image=True)
                name_obj_cond, lambda: load_json(
                    root, "images/val", "val.json", per_image=True)
