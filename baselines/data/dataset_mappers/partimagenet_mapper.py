...
from baselines.data.dataset_mappers.partimagenet_id_mapping import partimagenet_id_map
from baselines.data.datasets.register_partimagenet import CLASS_NAMES, OBJ_CLASS_NAMES

__all__ = ["PartImagenetDatasetMapper"]


# TODO:

class PartImagenetDatasetMapper:
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.id_map = partimagenet_id_map
    ...
    def __call__(self, dataset_dict):
        ...
        data_id = dataset_dict["file_name"].split('/')[-1].split('_')[0]
        dataset_name = dataset_dict["meta"]["dataset_name"]
        meta = MetadataCatalog.get(dataset_name)

        # Load obj segmentation map
        if 'obj_annotations' in dataset_dict:
            obj_name = self.id_map[data_id]
            obj_id = OBJ_CLASS_NAMES.index(obj_name)
            
            obj_annos = dataset_dict['obj_annotations']
            obj_part_sem_seg_gt = np.zeros(
                image_shape, dtype=np.float32) + self.ignore_label
            obj_sem_seg_gt = np.zeros(
                image_shape, dtype=np.float32) + self.ignore_label
            for i, anno in enumerate(obj_annos):
                part_name = meta.part_categories[anno['category_id']]['name'].split(' ', maxsplit=1)[1].lower()
                obj_part_name = "'s ".join([obj_name, part_name])
                obj_part_id = CLASS_NAMES.index(obj_part_name)
                
                mask = self.ann_to_mask(
                    anno, dataset_dict['height'], dataset_dict['width'])
                
                obj_part_sem_seg_gt[mask == 1] = obj_part_id
                obj_sem_seg_gt[mask == 1] = obj_id
        else:
            raise ValueError(
        ...
        if self.obj_part_map is not None:
            mapped_obj_part_sem_seg_gt = np.zeros_like(
                obj_part_sem_seg_gt, dtype=np.float) + self.ignore_label
            for old_id, new_id in self.obj_part_map.items():
                mapped_obj_part_sem_seg_gt[
                    obj_part_sem_seg_gt == old_id
                ] = new_id
            dataset_dict['ori_obj_part_sem_seg_gt'] = mapped_obj_part_sem_seg_gt

        aug_input = T.AugInput2(
            image, sem_seg=obj_sem_seg_gt, part_sem_seg=obj_part_sem_seg_gt)
        ...
        #     obj_sem_seg_gt[obj_sem_seg_gt !=
        #                    dataset_dict["category_id"]] = self.ignore_label
        #     obj_part_sem_seg_gt[obj_sem_seg_gt !=
        #                         dataset_dict["category_id"]] = self.ignore_label

        if self.obj_map is not None:
            mapped_obj_sem_seg_gt = np.zeros_like(
                obj_sem_seg_gt, dtype=np.float) + self.ignore_label
        ...
        if self.obj_part_map is not None:
            mapped_obj_part_sem_seg_gt = np.zeros_like(
                obj_part_sem_seg_gt, dtype=np.float) + self.ignore_label
            for old_id, new_id in self.obj_part_map.items():
                mapped_obj_part_sem_seg_gt[obj_part_sem_seg_gt ==
                                           old_id] = new_id
        ...
        if obj_part_sem_seg_gt is not None:
            obj_part_sem_seg_gt = torch.as_tensor(
                obj_part_sem_seg_gt.astype("long"))
        ...
        if self.size_divisibility > 0:
            ...
            if obj_sem_seg_gt is not None:
                obj_sem_seg_gt = F.pad(
                    obj_sem_seg_gt, padding_size, value=self.ignore_label).contiguous()

            if obj_part_sem_seg_gt is not None:
                obj_part_sem_seg_gt = F.pad(
                    obj_part_sem_seg_gt, padding_size, value=self.ignore_label).contiguous()
        ...
        if obj_sem_seg_gt is not None:
            ...
            # obj_mask = obj_part_sem_seg_gt != self.ignore_label
        ...
        if obj_sem_seg_gt is not None:
            ...
            obj_part_classes = obj_part_classes[obj_part_classes !=
                                                self.ignore_label]
            obj_part_instances.gt_classes = torch.tensor(
                obj_part_classes, dtype=torch.int64)
            ...
            if len(obj_masks) == 0:
                ...
                obj_instances.gt_masks = torch.zeros(
                    (1, obj_sem_seg_gt.shape[-2], obj_sem_seg_gt.shape[-1]))
                obj_instances.gt_classes = torch.tensor(
                    [-1], dtype=torch.int64)
            else:
                obj_masks = BitMasks(
                    torch.stack(
                        [torch.from_numpy(np.ascontiguousarray(x.copy())) for x in obj_masks])
                )
                obj_instances.gt_masks = obj_masks.tensor
                obj_instances.gt_classes = torch.tensor(
                    obj_classes, dtype=torch.int64)
            ...
            if len(obj_part_masks) == 0:
                # Some image does not have annotation (all ignored)
                obj_part_instances.gt_masks = torch.zeros(
                    (0, obj_part_sem_seg_gt.shape[-2], obj_part_sem_seg_gt.shape[-1]))
            else:
                obj_part_masks = BitMasks(
                    torch.stack(
                        [torch.from_numpy(np.ascontiguousarray(x.copy())) for x in obj_part_masks])
                )
